...
export default function WorkAmount() {
	...
	const handleClickAmount = (amount: string) => {
		...
		if (isActive) {
			setVacancyFilters({ ...vacancyFilters, worksAmount: '' })
		} else {
			setVacancyFilters({ ...vacancyFilters, worksAmount: amount })
		}
	}

	return (
		<div>
			<p className='text-[0.9375rem] leading-[1.125rem] mb-2.5'>
				Количество работ
			</p>
						...
						<div key={amount} className='flex items-center'>
							<button
								...
								className={cn(
									...
									{
										'border-[#A5C585]':
											amount.replaceAll('+', '') === vacancyFilters.worksAmount,
									}
								)}
							/>
