...
export function AuthModalContent({ onClose }: AuthModalContentProps) {
  ...
  return (
    ...
    >
      ...
      <CloseButton onClick={onClose} className='absolute right-[1.875rem] top-[2.375rem]' />
      ...
      {activeSection === 'introduce' && <IntroduceSection setActiveSection={setActiveSection} />}
      {activeSection === 'login' && <LoginSection setActiveSection={setActiveSection} />}
      {activeSection === 'reg' && <RegSection setActiveSection={setActiveSection} />}
      ...
      <div className='w-[36.8125rem] h-[5.3125rem] bg-primary-light absolute bottom-[1.8125rem] right-[5.375rem] pt-5 pl-[0.4375rem]'>
        <div className='text-primary-dark text-[0.8125rem] w-[32.8125rem] leading-tight text-center [&_span]:underline [&_span]:font-medium [&_span]:skip-ink-none'>
          <p>
            Продолжая, вы соглашаетесь с положениями основных документов <span>Workify</span>.
          </p>
          <p>
            Это <span>Условия предоставления услуг</span> и <span>Политика конфиденциальности</span>{' '}
          </p>
