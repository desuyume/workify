...
export const createFeedback = ({ params, config }: CreateFeedbackConfig) => {
  ...
  >(`/feedback/${params.vacancyId}`, params.data, config)
}
...
export const updateFeedback = ({ params, config }: UpdateFeedbackConfig) => {
  return authApiInstance.patch<
    any,
    {
      data: IFeedback
    }
  >(`/feedback/${params.vacancyId}/${params.feedbackId}`, params.data, config)
}
...
export const getVacancyFeedbacks = ({ params, config }: GetVacancyFeedbacksConfig) => {
  ...
  return apiInstance.get<{
    feedbacks: IFeedback[]
    count: number
  }>(`/feedback/vacancy/${params.vacancyId}${queryString}`, config)
}
