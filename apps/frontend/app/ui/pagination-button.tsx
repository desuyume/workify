...
export function PaginationButtonLeft({ href, isDisabled }: { href: string; isDisabled?: boolean }) {
  const className =
  ...
  return isDisabled ? (
    ...
    <Link href={href} className={cn(className, 'hover:bg-primary-light transition-colors group')}>
      <ArrowIcon variant='light' direction='left' />
    </Link>
  )
}
...
export function PaginationButtonRight({
  ...
  return isDisabled ? (
    ...
    <Link href={href} className={cn(className, 'hover:bg-transparent transition-colors group')}>
      <p className='text-[1.25rem] text-primary-dark group-hover:text-primary-light transition-colors mr-5'>
        Далее
      </p>
      <ArrowIcon variant='dark' direction='right' />
