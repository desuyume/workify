...
export function SettingInput({
	...
	const handleClickEdit = () => {
		if (settingType === 'name') {
			updateUserName({ params: { name: value } }).then(() => {
				setProfile({ user: { ...profile.user, name: value } })
		...
		if (settingType === 'birthday') {
			updateUserBirthday({
				params: { birthday: !!value ? new Date(value) : null },
			}).then(() => {
				setProfile({ user: { ...profile.user, birthday: value } })
				toast.success('Дата рождения успешно изменена')
			})
		}

		if (settingType === 'specialisation') {
			updateUserSpecialisation({ params: { specialisation: value } }).then(
				() => {
					setProfile({ user: { ...profile.user, specialisation: value } })
		...
		if (settingType === 'email') {
			updateUserEmail({ params: { email: value } })
				.then(() => {
					setProfile({ user: { ...profile.user, email: value } })
		...
		if (settingType === 'phone') {
			updateUserPhone({ params: { phone: value } }).then(() => {
				setProfile({ user: { ...profile.user, phone: value } })
				toast.success('Телефон успешно изменен')
			})
		}

		if (settingType === 'password') {
			updateUserPassword({ params: { password: value } }).then(() => {
				setProfile({ user: { ...profile.user, password: value } })
		...
		if (settingType === 'birthday') {
			setValue(
				profile.user.birthday
					? new Date(profile.user.birthday).toISOString().split('T')[0]
					: ''
			)
			setInitialValue(
				profile.user.birthday
...
export function SettingTextarea({
	...
	const clickEdit = () => {
		updateUserDescription({ params: { description: value } }).then(() => {
			setProfile({ user: { ...profile.user, description: value } })
