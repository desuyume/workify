import { generateSlug } from '@workify/shared'
...
function processDuplicatesSimple(cities: InitialCity[]): City[] {
  const nameCountMap = new Map<string, number>()

  cities.forEach((city) => {
    nameCountMap.set(city.name, (nameCountMap.get(city.name) || 0) + 1)
  })

  return cities.map((city) => {
    const count = nameCountMap.get(city.name)!

    if (count === 1) {
      return { ...city, slug: generateSlug(city.name) }
    } else {
      return {
        ...city,
        name: `${city.name} (${formatSubjectName(city.subject)})`,
        slug: generateSlug(`${city.name} (${formatSubjectName(city.subject)})`)
      }
    }
  })
}
...
function formatSubjectName(subject: string): string {
  ...
  // Пробуем найти сокращение
  for (const [full, abbr] of Object.entries(abbreviations)) {
    if (lowerSubject.includes(full)) {
      // Убираем тип субъекта из названия
      const nameWithoutType = subject.replace(new RegExp(full, 'i'), '').trim()
      return `${nameWithoutType} ${abbr}`.trim()
    }
  }
...
const INITIAL_RUSSIAN_CITIES: InitialCity[] = [
  {
    coords: {
      lat: '52.65',
      lon: '90.083333333333'
...
export const RUSSIAN_CITIES: City[] = processDuplicatesSimple(INITIAL_RUSSIAN_CITIES)
